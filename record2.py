...
if __name__ == '__main__':
    ...
    parser=OptionParser(option_class=eng_option, usage="%prog: [options]\nRecord 2 channels from USRP")
    parser.add_option("-f", "--freq", type="eng_float", default=89.7e6,
                      help="Center frequency [default=%default]")
    parser.add_option("--gain1", type="eng_float", default=28,
                      help="First channel gain in dB [default=%default]")
    ...
    rec = sdrutils.dual_recorder(samp_rate=options.sampling,
                                 freq=options.freq,
                                 gain1=options.gain1,
                                 gain2=options.gain2,
    ...
    try:
        ...
        pass
    if options.convert:
        filename_1,filename_2=rec.filename_1,rec.filename_2
        ...
        if options.clean:
            if options.verbose:
                print('Deleting %s and %s...' % (filename_1,filename_2))
        ...
        if options.verbose:
            ...
            print('Found %d samples in %s' % (out2[0].header['NITEMS'],
                                              filename_2))
        if options.merge:
            if out1[0].header['NITEMS']==out2[0].header['NITEMS']:
                newdata=np.c_[out1[0].data, out2[0].data]
                out1[0].data=newdata
                out1.verify('silentfix')
                outfits='%s.fits' % filenamebase
                if os.path.exists(outfits):
                    os.remove(outfits)
                out1.writeto(outfits)
                if options.verbose:
                    print('Wrote %s' % outfits)
            else:
                raise ValueError('Output of channels had different lengths')
